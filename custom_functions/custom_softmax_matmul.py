...
class softmax_matmul(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            # 打印数据类型，便于调试
            # print(f"grad_input1 dtype: {grad_input1.dtype}")
            # print(f"input1_sm dtype: {input1_sm.dtype}")

            # 确保 grad_input1 和 input1_sm 的数据类型为 torch.float32
            # grad_input1 = grad_input1.to(torch.float32)
            # input1_sm = input1_sm.to(torch.float32)
