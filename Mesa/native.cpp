...
std::tuple<at::Tensor, at::Tensor, at::Tensor> conv2d_backward(
    ...
    bool benchmark,
    bool deterministic,
#if TORCH_VERSION_MAJOR >= 1 && TORCH_VERSION_MINOR >= 7 || TORCH_VERSION_MAJOR > 1
    bool allow_tf32,
#endif
#if TORCH_VERSION_MAJOR < 2
...
at::Tensor gelu_backward_cpu(
    const at::Tensor& grad_output,
    const at::Tensor& input) {
#if TORCH_VERSION_MAJOR >= 1 && TORCH_VERSION_MINOR >= 10 || TORCH_VERSION_MAJOR > 1
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> layer_norm_forward_cpu(
    const at::Tensor & input,
#if TORCH_VERSION_MAJOR >= 1 && TORCH_VERSION_MINOR >= 8 || TORCH_VERSION_MAJOR > 1
...
#elif  TORCH_VERSION_MAJOR > 1
    return at::_softmax_backward_data(grad_output, output, dim, self.scalar_type());
#else
